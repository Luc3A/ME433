...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    volatile unsigned int delay_times[numBits]; // adjusted to be dependent on the number of LEDS
    ...
    for (i = 0; i < numLEDs; i++) {
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            // want to convert the unsigned char to binary
