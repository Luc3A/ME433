...
for i in range(len(reds)):
    #print(f"{raw[i]:#018b}")
    if ((reds[i] + greens[i] + blues[i]) > 128): #pixel is black
        grays.append(1)
    else:
        grays.append(0)
...
for i in range(len(grays)):
    #COM = sum(color * index) / sum(color)
    totalMass = totalMass + (grays[i] * i)
    totalColor = totalColor + grays[i]

COM = totalMass/totalColor
...
#if the center of mass is ~30, that is the middle :D
